...
const debug = require('debug')('jeny:db');
...
DB.prototype.select = function (table, options) {
    ...
    if (options && options.sort) {
        if (!Array.isArray(options.sort)) {
            return Promise.reject(new Error('sort passed to db.selectOne must be an array'));
        }
        R.forEach(column => {
            let asc = true;
            if (column[0] === '-') {
...
DB.prototype.count = function (table, options) {
    let sql = squel.select().from(table).field('COUNT(*) as count');
    sql = this.setWheres(sql, options.where);
...
DB.prototype.executeSquelAndReturnFirst = function (sql) {
    const paramQuery = sql.toParam();
    return this.query(paramQuery.text, R.map(v => v === 'null' ? null : v, paramQuery.values))
...
DB.prototype.update = function (table, where, what) {
    const params = R.fromPairs(R.filter(pair => {
